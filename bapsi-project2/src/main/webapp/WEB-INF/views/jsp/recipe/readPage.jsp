					...
					<div class="media-container-row title">
						<div class="col-12 col-md-11">
							...
							<div class="mbr-section-btn align-center">
								...
								<button id="oppose" 
								class="<c:out value="${checkDown == 0? 'mbri-sad-face mbr-iconfont mbr-iconfont-btn':'mbri-sad-face mbr-iconfont mbr-iconfont-btn active'}"/>"
								style="width: 220px; font-size: 18px;"><span
							...
							<td>
								<div style="width: 1000px" rows="3" cols="30">
									<input type="hidden" name="loginUserNo" id="writer"
										value="${userVO.no}" /> <input type="text"
				 ...
				 $.ajax({
					...
					data : JSON.stringify(data),
  					...
  					success : function(data) {
						alert(data.msg);
						if(data.checkDown==0) {
	                        $('#recommand').prop("class","mbri-sad-face mbr-iconfont mbr-iconfont-btn");
	                    }
						else{
	                        $('#recommand').prop("class","mbri-sad-face mbr-iconfont mbr-iconfont-btn active");
	                    }
					}, error: function(data){
						alert("문제가 발생했습니다. 다시 시도해주세요");
					}
      ...
      function getPgeNum(page) {
         $.getJSON("/bapsi/replies/" + recipeNo + "/" + page, function(data) {
            ...
            $(data.reList).each(function() {
                  str += "<li data-no='"+this.no+"' class='reList'>"
